...
(function($) {
    ...
    $.fn.persianDatePicker = function(opt) {
        var pluginName = 'persianDatepicker';
        var instance = this.data(pluginName);
        if (!instance) {
            return this.each(function() {
                return $(this).data(pluginName, new persianDatepicker(this, opt));
            });
        }
        return (opt === true) ? instance : this;
    }
    ...
    var persianDatepicker = (function($) {
        function persianDatepicker(element, userOptions) {
            ...
            var _fontSize = self.options.fontSize;
            ...
            var onResize = function(t) {
                var elPos = el.offset();
                self.calendar.css(
                        {
                            top: (elPos.top + el.outerHeight() + self.options.calendarPosition.y) + (t)+ 'px',
        ...
        persianDatepicker.prototype = {
            ...
            header: function() {
                var self = this;                
                var _monthYear = $('<div class="" />');
                _monthYear.appendTo(this.calendar);
                var _head = $('<div class="header" ' + self.headerStyle + ' />');
                _head.appendTo(this.calendar);
                var _next = $('<div class="nextArrow" />')
                        .html(this.options.nextArrow)
                        .attr('title', 'ماه بعد')
                        .bind("click", function() {
                            var nextMonth = self.persianDate.month + 1;
                ...
                var _prev = $('<div class="prevArrow" />')
                        .html(this.options.prevArrow)
                        .attr('title', 'ماه قبل')
                        .bind("click", function() {
                            var prevMonth = self.persianDate.month - 1;
            ...
            // days of week title
            dows: function() {
                var _row = $('<div class="dows" />');
                for (var i = 0; i < 7; i++) {
                    var _cell = $('<div class="dow cell " ' + this.cellStyle + ' />')
            ...
            content: function() {
                var self = this;
                var _days = $('<div class="days" />');
                _days.appendTo(this.calendar);
                var jd = self.persianDate;
                jd.date = 1;
                var _start = self.jDateFunctions.getWeekday(self.jDateFunctions.getJulianDayFromPersian(jd))
                var _end = self.jDateFunctions.getLastDayOfPersianMonth(self.persianDate);
                for (var row = 0, cellIndex = 0; row < 5 + 1; row++) {
                    var _row = $('<div />');
                    for (var col = 0; col < 7; col++, cellIndex++) {
                        if (cellIndex < _start || cellIndex - _start + 1 > _end) {
                            var _cell = $('<div class="nul cell " ' + self.cellStyle + ' />')
                                    .html('&nbsp;');
                        } else {
                            var _dt = self.getDate(self.persianDate, cellIndex - _start + 1);
                            var _today = '', _selday = '';
                            if (self.now().compare(_dt) == 0)
                                _today = 'today';
                            if(cellIndex - _start + 1==self.now().date)
                                _selday = 'selday';
                            var _fri = col == 6 ? 'friday' : '';
                            var _cell = $('<div class="day cell ' + _fri + ' ' + _today + ' ' + _selday + '" ' + self.cellStyle + ' />');
    ...
    var jDateFunctions = (function($) {
        ...
        jDateFunctions.prototype = {           
            ...
            getJulianDayFromPersian: function(pd) {
                var y0 = pd.year, m0 = pd.month, d0 = pd.date;
            ...
            getWeekday: function(jd) {
                ...
                var wd = Math.floor((jd + 1.5) % 7.0);
                return wds[wd];
            },
            getLastDayOfPersianMonth: function(pd) {
                var y = pd.year, m = pd.month;
                if (m >= 1 && m <= 6) {
                    return 31;
                }
                else if (m >= 7 && m < 12) {
    ...
    (function($) {
        ...
        Date.prototype._toString = function(formatDate) {
            var months = ["Januray", "February", "March", "April", "May", "June", "Julay", "August", "September", "October", "November", "December"];
            var dows = ["Sun", "Mon", "Tue", "Wed", "Tur", "Fri", "Sat"];
